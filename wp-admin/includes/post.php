...
function _wp_post_thumbnail_html( $thumbnail_id = NULL ) {
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		...
		if ( !empty( $thumbnail_html ) ) {
			...
			$content .= '<p class="hide-if-no-js"><a href="#" id="remove-post-thumbnail" onclick="WPRemoveThumbnail(\'' . $ajax_nonce . '\');return false;">' . esc_html__( 'Remove featured image' ) . '</a>
            </p>';
		}
    ...
    $topimg_pid = $_GET['post'];
    if(empty($topimg_pid)){
        $topimg_pid = $_REQUEST['post_id'];
    }
    $topimg_url = get_topimg($topimg_pid);
    if($topimg_url){
        $topimg_text = '<img width="266" height="73" title="都会对香 21-02" alt="都会对香 21-02" class="attachment-post-thumbnail" src="'.$topimg_url.'">';
        $content .= '<p class="hide-if-no-js" id="zhidingpic"><a title="' . esc_attr__( 'Set featured image' ) . '" href="' . esc_url( get_upload_iframe_src('image') ) . '" id="set-post-thumbnail" class="thickbox">'.$topimg_text.'</a></p>';
        $content .= '<p class="hide-if-no-js" id="removezhiding"><a href = "#" onclick = "deltoppic();return false;"> 删除置顶 </a></p>';
        $content .= '<script type="text/javascript">
                function deltoppic(){
                    jQuery.post(ajaxurl,{
							...
							if(jQuery("#zhidingpic")){
                            jQuery("#zhidingpic").slideUp("slow");
							jQuery("#zhidingpic").remove();
							jQuery("#removezhiding").html("<a title=\"设置特色图像\" href=\"media-upload.php?post_id='.$_GET['post'].'&amp;type=image&amp;TB_iframe=1\" id=\"set-post-thumbnail\" class=\"thickbox\">设置置顶图像</a>")
        ...
        $content .= '<p class="hide-if-no-js" id="zhiding"><a title="' . esc_attr__( 'Set featured image' ) . '" href="' . esc_url( get_upload_iframe_src('image') ) . '" id="set-post-thumbnail" class="thickbox">设置置顶图像</a></p>';
    }
