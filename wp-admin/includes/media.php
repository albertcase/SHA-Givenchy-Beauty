...
function get_media_item( $attachment_id, $args = null ) {
    ...
    $topimg_url = get_topimg($_GET['post_id']);
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
            ...
            if($topimg_url != $thumb_url){
                $top_img = wp_get_attachment_url($attachment_id);
                $img_click = "onclick = 'settoppic(\"$top_img\");return false;'";
                $delete = '<a href = "#" class="settoppic" '.$img_click.'> 图片置顶 </a>';
			}else{
                $delete = '';
			}
    ...
    $item .= '<script type="text/javascript">
               function settoppic(b){
                    jQuery.post(ajaxurl,{
                            action:"topimg_insert",
                            id:'.$_GET['post_id'].',
                            imgurl:b,
			...
			var dataObj=eval("("+data+")");
                            ...
                            jQuery(".settoppic").html("完成");
							
							jQuery(".settoppic").fadeOut(1000);
							
							var a=jQuery(window.parent.document.body).find("#zhiding").parent().find(".hide-if-no-js");
							var b=jQuery(window.parent.document.body).find("#removezhiding").parent().find(".hide-if-no-js");
							console.log(a.length,b.length);
							if(a.length<4&&b.length==0||b.length<4&&a.length==0){
								jQuery(window.parent.document.body).find("#zhiding").before("<p class=\"hide-if-no-js\" id=\"zhidingpic\"><a title=\'设置置顶\' href=\'media-upload.php?post_id='.$_GET['post_id'].'&type=image&TB_iframe=1&width=640&height=424\'><img width=\'266\' height=\'73\' src="+dataObj.data.url+"></a></p>");
								jQuery(window.parent.document.body).find("#zhiding").html("<a href=\"#\" onclick=\"deltoppic();return false;\"> 删除置顶 </a>")
								}
							if(a.length==4||b.length==4){
								jQuery(window.parent.document.body).find("#zhidingpic").html("<a title=\'设置置顶\' href=\'media-upload.php?post_id='.$_GET['post_id'].'&type=image&TB_iframe=1&width=640&height=424\'><img width=\'266\' height=\'73\' src="+dataObj.data.url+"></a>");
							}
