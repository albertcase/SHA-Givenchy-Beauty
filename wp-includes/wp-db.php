...
'/*/WP_I18N_DB_CONN_ERROR*/, $this->dbhost ), 'db_connect_fail' );
	...
	function insert( $table, $data, $format = null ) {
	    $this -> get_start_time();
        $rs = $this->_insert_replace_helper( $table, $data, $format, 'INSERT' );
        $this -> get_end_time();
        $this -> add_query_log('insert');
		return $rs;
	}
	...
	function update( $table, $data, $where, $format = null, $where_format = null ) {
        ...
        $rs = $this->query( $this->prepare( $sql, array_merge( array_values( $data ), array_values( $where ) ) ) );
        $this -> get_end_time();
        $this -> add_query_log('update');
	...
	function get_row( $query = null, $output = OBJECT, $y = 0 ) {
		...
		if ( $output == OBJECT ) {
            $rs = $this->last_result[$y] ? $this->last_result[$y] : null;
            $this -> get_end_time();
            $this -> add_query_log('get_row');
    		return $this->last_result[$y] ? $this->last_result[$y] : null;
		} elseif ( $output == ARRAY_A ) {
			$rs = $this->last_result[$y] ? get_object_vars( $this->last_result[$y] ) : null;
            $this -> get_end_time();
            $this -> add_query_log('get_row');
    		return $this->last_result[$y] ? get_object_vars( $this->last_result[$y] ) : null;
		} elseif ( $output == ARRAY_N ) {
			$rs = $this->last_result[$y] ? array_values( get_object_vars( $this->last_result[$y] ) ) : null;
            $this -> get_end_time();
            $this -> add_query_log('get_row');
	...
	function get_results( $query = null, $output = OBJECT ) {
		...
		if ( $output == OBJECT ) {
			...
			$this -> get_end_time();
            $this -> add_query_log('get_results');
    ...
    //type update get_row get_results insert
    /**
	 * 记录查询日志
	 * @param $type  查询方法的类型 update get_row  get_results  insert
	 * @return 
	 */
    function add_query_log($type = ''){
        if(defined('WP_DB_QUERY_LOG') && WP_DB_QUERY_LOG === true){
            //$log_root =  ABSPATH . 'wp-query-log/'.$type.'/';
            $log_root =  ABSPATH . 'wp-query-log/';
            //$filename = $log_root.date('Y_m_d').'_'.$type.'.csv';
            $filename = $log_root.date('Y_m_d').'.csv';
            $data = array(
                'url' => '"'.$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'].'"',
                'start_time' => $this -> query_log_start_time,
                'end_time' => $this -> query_log_end_time,
                'time_total' => $this -> query_log_end_time - $this -> query_log_start_time,
            ...
            error_log($log_content,3, $filename);
        }
    }
}
